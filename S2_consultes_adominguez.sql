...
WITH transaccions_ordenades AS (     #s'obre la CTE
	SELECT 
		t.card_id,
        t.declined,
		ROW_NUMBER() OVER (
		PARTITION BY t.card_id       #separem per grups sense eliminar rows
		ORDER BY d.full_date DESC
		) AS ordre_transaccio        #1, 2, 3, (aquí farem stop a la nostra consulta principal amb filtre), 4, 5...
FROM transactions t
JOIN dim_date d
ON t.date_key = d.date_key
)                                    #es tanca la CTE
